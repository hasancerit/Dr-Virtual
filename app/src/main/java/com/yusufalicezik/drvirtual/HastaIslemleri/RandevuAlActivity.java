...
public class RandevuAlActivity extends AppCompatActivity {
    ...
    private int kaydedilecekGun;
    private int kaydedilecekSaat;
    private  String eskiDetaylar="";
    ...
    private void doktorDetayVerileriAl() {
                ...
                if (e == null) {
                       ...
                       // Log.e("tttt",scoreList.get(0).getString("doktor_alinan_randevular"));
                        if(scoreList.size()>0) {
                            ...
                            eskiDetaylar = scoreList.get(0).getString("doktor_alinan_randevular");
    ...
    public void doktorDetayKaydet() {
        if (eskiDetaylar.equals("")) {
            final ParseObject object=new ParseObject("doktor_detay");
            object.put("doktor_tc",secilenDoktor.getDoktorTc());
            object.put("doktor_alinan_randevular",kaydedilecekGun + "-" + kaydedilecekSaat);
            ...
            query.findInBackground(new FindCallback<ParseObject>() {
                public void done(List<ParseObject> scoreList, ParseException e) {
                    if (e == null) {
                        if (scoreList.size() > 0) {
                            scoreList.get(0).put("doktor_tc", secilenDoktor.getDoktorTc());
                            scoreList.get(0).put("doktor_alinan_randevular", eskiDetaylar + "," + kaydedilecekGun + "-" + kaydedilecekSaat);
